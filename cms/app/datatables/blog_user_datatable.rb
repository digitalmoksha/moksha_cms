...
private
  ...
  def action(user)
    if user.has_role? :member, @blog
      ...
      link_to(icons("icon-plus"), url_helpers.blog_add_member_admin_cms_blog_path(@blog, :locale => DmCore::Language.locale, :user_id => user.id), :title => 'Add Access') + "&nbsp;&nbsp;".html_safe + h(user.full_name)
    end
  end
end
