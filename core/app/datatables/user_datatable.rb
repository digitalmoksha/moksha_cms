...
private
  ...
  def fetch_users
    ...
    if params[:sSearch].present?
      users = users.where("LOWER(user_profiles.first_name) like LOWER(:search) OR LOWER(user_profiles.last_name) like LOWER(:search) OR LOWER(users.email) like LOWER(:search)", search: "%#{params[:sSearch]}%")
    end
    users
  end
