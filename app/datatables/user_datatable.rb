...
private
  ...
  def fetch_users
    ...
    if params[:sSearch].present?
      users = users.where("LOWER(user_profiles.first_name) like :search OR LOWER(user_profiles.last_name) like :search OR LOWER(email) like :search", search: "%#{params[:sSearch]}%")
    end
    users
  end
