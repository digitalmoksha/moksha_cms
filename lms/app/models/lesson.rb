...
class Lesson < ApplicationRecord
  ...
  scope :next,            ->(row_order, course_id) { where("row_order > ? AND course_id = ?", row_order, course_id) }
  scope :previous,        ->(row_order, course_id) { where("row_order < ? AND course_id = ?", row_order, course_id) }
